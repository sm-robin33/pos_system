...
$purchase = "SELECT * FROM draft_parchase WHERE referance='$referance_id'";
...
FROM draft_parchase_details AS ps
INNER JOIN draft_parchase AS p ON p.supplier_name = ps.supplier_id and ps.referance = p.referance
...
if (isset($_POST['update'])) {
    ...
    foreach($quantities as $quantities_check){
        if($quantities_check<0){
            ...
            header("Location:index.php?page=edit_draft");
            exit;
        }
    }
    ...
    if ($conn->query($query1)) {
        $query11 = "DELETE FROM draft_parchase WHERE referance = $referance_id";
        $conn->query($query11);
        $query12 = "DELETE FROM draft_parchase_details WHERE referance = $referance_id";
        $conn->query($query12);
...
if (isset($_POST['draft'])) {
    ...
    for ($i = 0; $i < count($sn); $i++) {
        ...
        $query2 = "UPDATE draft_parchase_details SET supplier_id = '$suppler_name', parchase_date = '$purchase_date',referance = '$referance',product_name='$product_name',quantity = '$quantity',unit_price='$unit_price',sub_total_price='$sub_unit_price'  WHERE referance=$referance_id and id = $pd_id[$i]";
        mysqli_query($conn, $query2);
        if($i>=$pd_id_no){
        $query4 = "INSERT INTO draft_parchase_details (supplier_id, parchase_date,referance,product_name, quantity, unit_price, sub_total_price) VALUES ('$suppler_name', '$purchase_date','$referance', '$product_name', '$quantity', '$unit_price', '$sub_unit_price')";
        mysqli_query($conn, $query4);
        }
    }
    $query1 = "UPDATE draft_parchase SET supplier_name = '$suppler_name', parchase_date = '$purchase_date',referance = '$referance',total_amount='$total_amount' WHERE referance=$referance_id ";


    if ($conn->query($query1)) {
        $_SESSION['status'] = "Draft Updated";
                            ...
                            <div class="col-md-12">
                                ...
                                <form action="" class="mt-3" id="add_name" method="POST">
                                    ...
                                    <div class="row mt-5">
                                        ...
                                        <div class="col-md-4"><input type="submit" class="samplewidth btn btn-success" name="update" id="submit" value="Save And Add"></div>
